...
- `DEV-ENVIRONMENTS.md` — how we test safely
- `DISCOVERY-CONTRACT.md` — Layer 1 spec
- `SETTINGS-SCHEMA.md` — broadsheet.json shape
- `RENDERER-CONTRACT.md` — plugin API sketch
- `ADDON-MOCK.md` — packaging
- `SETTINGS-UI.md` — Settings screens
- `PREMORTEM-DIFF.md` — research findings
- `PUBLIC-README-DRAFT.md` — public pitch (the v0.1 readme)
...
editorial dashboard for whatever HA tells it about — areas, entities,
people — with a Settings UI for hide/pin/rename curation, no
...
- **Pages**: `/` (landing manifest), `/lights`, `/heat`, `/door`, `/tv`
  (without TMDB content rows — those are a v0.1 plugin), `/body`,
...
- **Responsive**: phone, desktop, wall tablet — three width axes
...
- **broadsheet HA theme**: an HA theme file the add-on drops into
  ...
  replacement vision — see `REPLACEMENT-VISION.md`. (Generalises
...
- **Lovelace importer** (v0.2) - on-ramp converting built-in Lovelace
  ...
  `REPLACEMENT-VISION.md`.
- **Theme inheritance from HA** — i.e. broadsheet *adopting HA's*
  ...
  confused — one is broadsheet wearing HA's clothes, the other is HA
  wearing broadsheet's.)
- **broadsheet as the outer shell** (HA's chrome never seen, HA config
  pages embedded inside broadsheet) — the structural half of the
...
- **Plugin auto-install via UI** (security risk — Settings UI shows
...
### M0 — Documentation freeze + scaffolding
...
1. All eight documentation files in `harold-home/docs/` are complete
   ...
   ADDON-MOCK, SETTINGS-UI). ← we are here
...
### M1 — Safety rails + WebSocket client

**Duration**: 2 days

**Tasks**:
1. Stand up `src/lib/ha/client.ts` based on
   `home-assistant-js-websocket` library (don't roll our own — see
...
- Try to call `light.turn_on` from dev console → blocked + audit logged
- Add `?allow-writes=true` → call succeeds, light toggles, audit logged
- Try to call `lock.unlock` even with `?allow-writes=true` → blocked
- Force HA restart via Proxmox → broadsheet detects disconnect within
  ~40s (heartbeat) and reconnects within ~10s of HA being back
- Audit log file shows expected `blocked-write` / `call-service` /
  `hard-banned` entries

### M2 — Discovery layer (Layer 1 + Layer 2)

**Duration**: 4 days

**Tasks**:
1. Implement `src/lib/discovery/registries.ts` — pull area, device,
   entity, floor, label, category registries. Subscribe to all
   `*_registry_updated` events.
2. Implement `src/lib/discovery/domain.ts` — Layer 2 projection. The
   `Floor → Area → Entity` hierarchy with the entity-area-fallback
...
3. Implement `src/lib/discovery/heuristics.ts`:
   ...
   - Presence-sensor picking per person (the `★ best` recommendation
     from SETTINGS-UI)
   - TV detection (`media_player.*` with device class `tv`)
   - Health-Connect sensor pattern matching for `/body`
4. Implement `src/lib/discovery/page-map.ts` — domain → page routing.
...
**Exit criteria**:
- Print discovered Areas to console at boot — count matches HA's
  area count
- Each Area's `lights / climates / locks / etc.` arrays populated
  correctly for at least 3 known rooms (Office, Living Room, Hallway)
- Floor → Area mapping correct (Ground / Upstairs / Unassigned)
- Labels surfaced as orthogonal tags
- Adding a new area in HA → broadsheet reflects it within 5s without
  refresh
- Renaming an area in HA → broadsheet's domain model picks up the
  rename within 5s
...
### M3 — Page templates (the six core pages)
...
1. Build `PageShell.svelte`, `Hero.svelte`, `Eyebrow.svelte`,
   ...
   `PinnedSection.svelte`, `UnsortedSection.svelte` — the layout
   primitives, copied from harold-home but generalised.
2. `/` — landing manifest. Procedural ambient gradient per detected
   area (no plugin dependency for v0.1 default). One-line manifest
   string. Default copy "[Person] is home in the [Room]." composable.
3. `/lights` — prose state, scene chips, per-area reveal with sliders
   + per-bulb sub-reveal.
4. `/heat` — three macros (Boost / All warm / All frost), per-room
   TRV reveal with ±0.5° nudges.
5. `/door` — lock state hero (read-only-display in dev) + Unlock
   action (gated). Camera image below if paired camera detected.
6. `/tv` — remote on left, app launcher on right. Content browser
   surface (TMDB section is a plugin slot — empty in core v0.1).
7. `/body` — health-data panels with honest sub-labels, stale banner.
8. `/wall` — dense action grid for hallway tablet (3 BIG primary
...
- A page is empty when no entities of its domain exist (e.g. `/door`
  when no `lock.*` discovered) — shows a graceful empty state, not
  a crash
...
### M4 — Curation (Layer 3) + Settings UI

**Duration**: 7 days

**Tasks**:
1. Define `broadsheet.json` schema v1 (see SETTINGS-SCHEMA.md)
2. Implement `src/lib/discovery/curation.ts` — Layer 3 overrides
   applied to Layer 2 output
3. Build `/settings` landing with alerts panel (the 7+3 alert classes
   from SETTINGS-UI + PREMORTEM-DIFF)
4. Build `/settings/house` — areas + entities curation (the hardest
   screen)
5. Build `/settings/people` — presence sensor picking with `★ best`
   recommendation
6. Build `/settings/voice` — editorial string overrides
7. Build `/settings/paintings` — per-area image upload + procedural
   fallback
8. Build `/settings/integrations` — TMDB key, Health Connect detection
9. Build `/settings/plugins` — list installed plugins, enable/disable
   (no auto-install in v0.1)
10. Build `/settings/about` — version, support bundle generator
...
**Exit criteria**:
- Hide an entity in Settings → it disappears from its page within 1s
- Pin an entity to a non-default page → it appears there within 1s
- Rename an area in Settings → header updates everywhere within 1s
- Pick a different presence sensor for a person → landing manifest
  updates within 5s
- Edit a voice string → renders new copy within 1s
- Stop the SPA, restart, reload → all curation persists
- Support bundle generates a downloadable zip with discovery state +
  redacted config + connection log

### M5 — HA add-on packaging
...
**Exit criteria**:
- Add `addon` repo URL to Env 2's HA → addon appears in
  store
- Install completes in <60s
- Open Web UI → broadsheet loads with live state, no token paste
- Toggle a (non-banned) entity → service call fires, state updates
- Restart the add-on container → curation file survives, SPA
  reconnects
- Bump `version: "0.1.0"` → "0.1.1" → push → HA shows update badge,
  apply update succeeds
- aarch64 image builds in CI (test via QEMU at minimum)

### M6 — Production canary (Env 3)
...
5. Use broadsheet personally for ≥7 days (open from sidebar, navigate,
...
### M7 — Public release prep

**Duration**: 4 days

**Tasks**:
1. Write public README from `PUBLIC-README-DRAFT.md` — strip
...
7. Tag v0.1.0 on both repos → CI publishes images to GHCR
...
M2 (discovery layer)        ←──┐
  ↓                            │
M3 (page templates)            │ both block on M2
  ↓                            │
M4 (curation + Settings UI) ←──┘
  ↓
M5 (add-on packaging)
  ↓
...
What could derail v0.1, ranked by likelihood × impact:
...
| `home-assistant-js-websocket` library missing a feature we need | Low | Medium | Fork or copy the relevant module — library is MIT |
...
| Take longer than 3 months elapsed | High | Low | Doesn't matter — there's no external deadline. Ship when ready. |
...
2. **Premature optimisation**: spending three weeks on perfect
   ...
   Mitigation: M2 has bounded exit criteria — "sensible Area[]
...
5. **Skipping the canary**: thinking Env 2 testing is enough.
   It isn't — the test HA has no real entities, no real life
...
- ≥1 external user (friend, beta tester) successfully installed
  broadsheet against their HA
- Issue tracker open, Discussions enabled, response cadence
  established (commit to ≤72h triage)
