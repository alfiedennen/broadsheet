...
Adapts to whatever you already have in HA â€” areas, lights, climates,
locks, media players, sensors. No `house.yaml` to write before it works.
Install, point at your HA, see your house.

[ screenshot â€” landing page, painting left, manifest text right, italic amber over warm off-black ]
...
- Home Assistant **OS** or **Supervised** install (the kinds that have
  the add-on store). Most people run HA OS â€” if you installed via the
  official HA image on a Pi, NUC, or a VM, you're set.
- HA 2024.4 or newer.

> **Running HA Container or HA Core?** v0.1 doesn't ship for those (no
> Supervisor = no add-on store). Docker support is on the roadmap â€”
...
2. In **Settings â†’ Add-ons â†’ Add-on Store**, find **broadsheet**, click
   **Install**.
3. Click **Start**, then **Open Web UI**.

That's it. The add-on auto-authenticates against your HA via the
Supervisor token â€” no LLAT to paste, no `.env` to fill in. broadsheet
...
cd broadsheet
pnpm install
pnpm dev               # localhost:5173 â€” paste HA URL + token in the setup form
...
with a long-lived token you generate in HA â†’ Profile â†’ Long-Lived
Access Tokens.
...
Eight pages, each shaped for what it's actually for â€” not a uniform grid
...
| **`/heat`** | Three macros (Boost / All warm / All frost), per-room TRV reveal with Â±0.5Â° nudges | Areas containing any `climate.*` |
...
| **`/body`** | Health-data panels with honest sub-labels ("once daily" / "measured during sleep") so empty panels explain themselves | Health Connect `sensor.*` pattern matching (Pixel) â€” Apple Health bridge planned |
...
hallway tablet â€” same app, surface-specific landing.
...
Tile, Tunet, the Lovelace strategy generators â€” there's a mature,
well-loved set of options for laying out tiles, sliders, and status
chips, and most of them are excellent at what they do.

broadsheet is a different shape â€” not because the others are wrong, but
...
isn't an upgrade â€” it's a stylistic alternative for the cohort that
...
- **House** â€” every area + entity broadsheet found, with hide / pin /
  reorder / rename. Drag entities between pages if our auto-grouping
  guessed wrong. No YAML.
- **People** â€” broadsheet finds your `person.*` entities; for each, pick
  the presence sensor you actually trust. (Warning surfaced for iOS
  Companion-App GPS-suspension cases â€” long-running pain that bites
  Pixel/iPhone mixed households.)
- **Voice** â€” the editorial strings. Default copy ships in English; you
  override per-section. Want your manifest line to be a haiku? Up to you.
- **Paintings** â€” the visual centre of `/`. Defaults to a procedural
  ambient gradient per area. Optional: drop PNGs into the data volume
  per the `<area-slug>.png` convention to use your own art.
- **Integrations** â€” TMDB key for `/tv` content (free, regional UK/US/EU
...
edit it directly if you prefer files to UIs â€” the Settings panel is a
thin wrapper.
...
- **SvelteKit 2 + Svelte 5 runes**, `adapter-static` â€” pure-static
...
- **Single persistent WebSocket** per device, owned by the layout. Auto-
  ...
  (30s ping / 10s pong-timeout / force-close on zombie) â€” survives HA
  restarts cleanly.
...
- **PWA** â€” manifest, icons, iOS apple-touch-icon. Add to Home Screen
  on iPhone or Android and it opens borderless.
- **Three responsive axes** â€” width, height, and hover-vs-touch. Tap
...
selling, that's fine â€” MIT license, as long as the colophon credit
...
extracted, generalised version â€” same paradigm, none of the personal
specifics.
